...
parameters specifying how a set of events and sensors (the
...
% below.
In each experiment we generated 1,000 settings with
...
%%   number of benign event prototypes, and distribute them among the
...
%%   necessary number of benign event prototypes to satisfy both
...
% benign prototype. For each prototype, if a sample of a Bernoulli
% random variable whose success probability is the occurrence
% probability of the prototype is 1, then the ground truth aspect will
% contain a sensor report with that prototype and location.
% %
% For sensor reports at each location, we consider each sensor report
% prototype of the setting. If a prototype indicates that the sensor can
...
%   negative probability of the report prototype. If \emph{any} of these
%   samples are 0 --- that is, if for any event we do \emph{not} have a
%   false negative --- then the ground truth aspect will contain a
%   sensor report with that prototype and location.
% \item If there are no events at a location which could trigger a
%   report of that prototype, then we consult a Bernoulli random
%   variable whose success probability is the false positive probability
%   of the sensor report prototype; if it returns 1, then the ground
%   truth aspect will contain a sensor report with that prototype and
%   location.
% \end{compactitem}
